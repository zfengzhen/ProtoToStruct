...
const char* TimeName(time_t t) {
  ...
  strftime(time_stamp, sizeof(time_stamp), "%Y-%m-%d %H:%M:%S", localtime(&t));
...
int32_t ParseParam(int32_t argc, char* argv[], vector<string>& include_path_vec,
                   vector<string>& proto_file_vec, string& out_path) {
  static struct option long_options[] = {{"include_path", 1, NULL, 'I'},
                                         {"proto_file", 1, NULL, 'P'},
                                         {"out_path", 1, NULL, 'O'},
                                         {0, 0, 0, 0}};
  ...
  while (-1 != (opt = getopt_long(argc, argv, "I:P:O:", long_options, NULL))) {
    switch (opt) {
      ...
      default: {
        cout << "Usage: " << argv[0]
             << "-I include_path -P proto_file -O out_path" << endl;
        return -1;
      }
    }
  }
...
int32_t ParseProtoFileDesc(Importer* importer, vector<string>& proto_file_vec,
  ...
  for (uint32_t i = 0; i < proto_file_vec.size(); i++) {
    ...
    for (int j = 0; j < count; j++) {
      ...
      if (m == true) {
        file_desc_map.insert(
            FileDescMap::value_type(proto_file_vec[i], file_desc));
        break;
      }
    }
  }
...
string GetStructFileFullName(string& out_path,
                             const FileDescriptor* file_desc) {
  string name = GetStructFileName(file_desc);
  if (out_path.size() == 0) return name;
...
void ParseInclude(const FileDescriptor* file_desc, FileDescMap& file_desc_map,
                  ostringstream& str) {
  str << "#pragma once\n\n" << endl;
      ...
      for (int32_t j = 0; j < field_count; j++) {
        ...
        if (cpp_type == FieldDescriptor::CPPTYPE_STRING) {
          ...
          if (field_options.HasExtension(string_max_size)) {
            str << "#define " << GetStringMaxSize(field_desc) << " "
                << field_options.GetExtension(string_max_size) << endl;
          } else {
            str << endl
                << field_desc->name()
                << " is string but not has string_max_size!" << endl;
            return;
          }
        }
        // repeated最大值
        if (label == FieldDescriptor::LABEL_REPEATED) {
          ...
          if (field_options.HasExtension(repeated_max_size)) {
            str << "#define " << GetRepeatedMaxSize(field_desc) << " "
                << field_options.GetExtension(repeated_max_size) << endl;
          } else {
            str << endl
                << field_desc->name()
                << " is repeated but not has repeated_max_size!" << endl;
            return;
          }
        }
      }
...
void ParseAssignStructToProto(const FieldDescriptor* field_desc,
                              ostringstream& str) {
  FieldDescriptor::CppType cpp_type = field_desc->cpp_type();
  FieldDescriptor::Label label = field_desc->label();
  string field_name = field_desc->lowercase_name();
  ...
  if (cpp_type != FieldDescriptor::CPPTYPE_MESSAGE &&
      label != FieldDescriptor::LABEL_REPEATED) {
    // 原始类型非repeated变量
    str << "\tproto->set_" << field_name << "(sct->" << field_name << ");"
        << endl;
  }
}

void ParseAssignProtoToStruct(const FieldDescriptor* field_desc,
                              ostringstream& str) {
  FieldDescriptor::CppType cpp_type = field_desc->cpp_type();
  FieldDescriptor::Label label = field_desc->label();
  string field_name = field_desc->lowercase_name();
  ...
  if (cpp_type == FieldDescriptor::CPPTYPE_STRING &&
      label != FieldDescriptor::LABEL_REPEATED) {
    // 原始类型非repeated变量
    str << "\tstrncpy(sct->" << field_name << ", proto->" << field_name
        << "().c_str(), " << GetStringMaxSize(field_desc) << ");\n";
  }
}
...
void ParseFuncStructToProto(const FileDescriptor* file_desc,
                            ostringstream& str) {
  int32_t count = file_desc->message_type_count();
  for (int i = 0; i < count; i++) {
    const Descriptor* msg_desc = file_desc->message_type(i);
    ...
    if (need_parse == true && NeedParseAssign(msg_desc) == true) {
      str << "void ConvPrimitiveStructToProto(" << GetStructName(msg_desc)
          << "* sct, " << msg_desc->name() << "* proto)\n{" << endl;
      int32_t field_count = msg_desc->field_count();
      for (int32_t j = 0; j < field_count; j++) {
        const FieldDescriptor* field_desc = msg_desc->field(j);
...
void ParseFuncProtoToStruct(const FileDescriptor* file_desc,
                            ostringstream& str) {
  int32_t count = file_desc->message_type_count();
  for (int i = 0; i < count; i++) {
    const Descriptor* msg_desc = file_desc->message_type(i);
    bool need_parse = msg_desc->options().GetExtension(parse_to_struct);
    if (need_parse == true && NeedParseAssign(msg_desc) == true) {
      str << "void ConvPrimitiveProtoToStruct(" << msg_desc->name()
          << "* proto, " << GetStructName(msg_desc) << "* sct)\n{" << endl;
      int32_t field_count = msg_desc->field_count();
      for (int32_t j = 0; j < field_count; j++) {
        const FieldDescriptor* field_desc = msg_desc->field(j);
...
int main(int argc, char* argv[]) {
  ...
  if (ret != 0 || proto_file_vec.size() == 0) {
    cout << "ret = " << ret
         << " proto_file_vec.size() = " << proto_file_vec.size() << endl;
    return -1;
  }
  ...
  // 遍历文件，找出需要解析的proto文件
  for (FileDescMap::iterator it = file_desc_map.begin();
       it != file_desc_map.end(); ++it) {
